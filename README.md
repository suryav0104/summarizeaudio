...
- **Summarize Text** — feed an existing transcript `.txt` file directly to the AI for a summary
...
4. Downloads the **gemma3:4b** AI model (~3 GB, runs on 8 GB RAM)
...
| Summarize Text | Opens a file picker — choose a `.txt` transcript to summarize. |
...
```toml
[ollama]
model = "gemma3:4b"       # AI model to use
