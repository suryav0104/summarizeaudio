...
### `requests` `timeout=` is a read timeout, and a cold model load can blow past it
`summarizer._summarize_prompt` posts to `/api/generate` with `timeout=120`. That value is the *read* timeout (time to first byte), not a total budget. The first request after Ollama starts (or after a different model was last used) has to load the model into memory before it emits any token — for `gemma3:12b` that cold load measured **~20s** in isolation but can exceed 120s under memory pressure or with a larger model, surfacing as `requests.exceptions.ReadTimeout` ("Read timed out. (read timeout=120)"). Two fixes, complementary:
- **Classify it honestly.** `error_handler.friendly_message` matches `"read timed out"` / `"readtimeout"` and returns a model-loading-specific message ("Ollama took too long to respond … the first time a model runs and has to load into memory … try again"). This branch sits *before* the unreachable branch, because a read timeout means Ollama IS reachable (don't mislabel it "server not reachable"). Note the cloud-sync branch matches `"operation timed out"`, which a read-timeout string does not contain, so there's no false match.
- **Prewarm to move the cold load off the critical path.** `ollama_client.prewarm(host, model)` sends a tiny non-streaming `/api/generate` that asks the model to echo a known statement verbatim and confirms a non-empty reply (returns `bool`, never raises). `prewarm_async` runs it on a daemon thread, fire-and-forget. Wired at three points: end of recording (`tray._on_stop_recording`), and the start of `pipeline._run_inner` for LOCAL_AUDIO (overlaps the load with transcription) and LOCAL_TEXT (overlaps with the file copy/read). RECORD is gated out of the pipeline prewarm because the tray already fired it — avoids a redundant request. Measured effect: cold prewarm 20.6s, second (warm) call 2.1s, so by summarize time the real request answers well under the timeout.
