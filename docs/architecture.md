...
| Settings window | `settings_window.py` | Modal Tk window with readonly dropdowns: Input Audio device, Summarization model, Speaker Diarization, and (macOS only) Open at Login. Reached from the single `Settings…` tray item. Stacks on top of Workflow/History (not subject to the one-window-at-a-time rule). Apply mutates config in place, calls `save_config`, posts `("rebuild_tray_menu",)`. |
