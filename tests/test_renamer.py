...
def test_mode1_renames_and_moves_mp3_and_txt(tmp_output):
    ...
    assert paths.audio.name == f"Audio_GTC Keynote_{today()}.mp3"
    assert paths.transcript.parent.name == "TranscriptionFiles"
    assert paths.transcript.name == f"Transcript_GTC Keynote_{today()}.txt"
...
def test_collision_appends_counter(tmp_output):
    ...
    (tmp_output / "AudioFiles" / f"Audio_test_{today()}.mp3").write_bytes(b"old_audio")
    (tmp_output / "TranscriptionFiles" / f"Transcript_test_{today()}.txt").write_text("old_txt")
    ...
    assert paths.audio.name == f"Audio_test_{today()}_2.mp3"
    assert paths.transcript.name == f"Transcript_test_{today()}_2.txt"
...
def test_collision_detected_from_transcript_only(tmp_output):
    ...
    (tmp_output / "TranscriptionFiles" / f"Transcript_notes_{today()}.txt").write_text("x")
    txt = tmp_output / "b.txt"
    txt.write_text("new")
    r = Renamer(tmp_output)
    paths = r.rename_session("notes", txt_path=txt)
    assert paths.transcript.name == f"Transcript_notes_{today()}_2.txt"
    assert paths.summary.name == f"Summary - notes_{today()}_2.md"
...
def test_mode3_copies_txt_does_not_move(tmp_output):
    ...
    assert paths.transcript.name == f"Transcript_notes_{today()}.txt"
...
def test_summary_path_uses_same_stem(tmp_output):
    ...
    assert summary_path.name == f"Summary - GTC Keynote_{today()}.md"
